/** \file
 * \brief RAII wrapper around PARDISO.
 *
 * \attention Including this file causes a compile time error unless either the macro
 * `PARDISO_STANDALONE` or `PARDISO_MKL` is defined. Those macros toggle
 * which version of PARDISO is used as a backend.
 *
 * \see The documentation of the Pardiso namespace contains more information.
 *
...
// There is no header included in standalone PARDISO,
// declare the two relevant functions manually.
...
#else
// Use the header provided in MKL.
...
#else
  #if defined(__GNUC__)
    #define PARDISO_MKL_UNUSED(x) UNUSED_ ## x __attribute__((unused))
 ...
 * version 5.0.0) or the MKL version. All differences are abstracted away for basic
 * usage. However, if need be, some version specific features can be used through
 * an interfaces in line with the rest of the API (see e.g. Pardiso::DParm).
 *
 ...
 * The simplest way to solve an equation of the form `mat*x = rhs` for `x` is the following
 ...
 // initialise mat and rhs

 Pardiso::State<double> pstate;
 blaze::Vector<double> x = pstate(mat, rhs);
 \endcode
 * More direct control is possible through the lower level call operators of
 * Pardiso::State.
 * Pardiso::Matrix is a helper for constructing matrices in the CRS layout required
 * by PARDISO.
 *
...
namespace Pardiso {
    ...
    /**
     * Encodes the symmetry type of matrices only. The datatype is determined based
    ...
    /// A single PARDISO execution phase.
    /**
     ...
     */
    enum class Phase {
        ANALYSIS = 1,      ///< Analysis of the szstem.
    ...
    /**
     * This class helps construct matrices in the CRS (compressed row storage) format
     ...
     * The only exceptions are `ia()` and `ja()` which give direct access to the
     * 1-based indices passed on to PARDISO.
     *
     * <B>Usage</B><BR>
     * You need to specify elements in order of ascending row and column.
     * The following shows an example using a matrix of an imaginary class
     * (this interface is implemented by blaze::CompressedMatrix) to
     * demonstrate how to fill an instance or Pardiso::Matrix.
     * \code
     ...
     \endcode
     * You <I>must</I> call Matrix::finishRow() when moving on to the next row
    ...
    struct Matrix {
        ...
        explicit Matrix(const blaze::CompressedMatrix<elementType> &mat)
        ...
        Matrix &operator=(const blaze::CompressedMatrix<elementType> &mat) {
            this->clear();
            reserve(mat.nonZeros(), mat.rows());
            assignFrom(mat);
            return *this;
        }
        ...
        /// Finalise the current row, prepare for input of next row.
        void finishRow() {
            _ia.push_back(static_cast<int>(_ja.size())+1);
        }

        /// Add an element to the current row.
        /**
         * \attention Must be called with `column` in ascending order.
         * \param column Index of the column.
         * \param value Element at given index.
         */
        void add(const int column, const elementType value) {
            _ja.push_back(column+1);
            _a.push_back(value);
        }
        ...
        elementType *a() noexcept {
            return &_a[0];
        }
        /// Get a pointer to the elements.
        const elementType *a() const noexcept {
            return &_a[0];
        }
    ...
    private:
        std::vector<elementType> _a;  ///< Array of matrix elements.
        std::vector<int> _ja;  ///< Array of column indices (1-based).
        std::vector<int> _ia;  ///< Array of row indices (1-based).

        /// Assign contents from a blaze::CompressedMatrix.
        /// Matrix must be empty when calling this function.
        void assignFrom(const blaze::CompressedMatrix<elementType> &mat) {
    ...
    /**
     * Initialises PARDISO on construction and cleans up PARDISO's internal memory
     * when it is destroyed.
     * Since the content of the internal state is unknown, it cannot be copied.
     * So in order to avoid shared references, %State can only be moved, not copied.
     *
     ...
     * the corresponing iparm. For MKL PARDISO, the number of threads is
     * determined by <TT>MKL_NUM_THREADS</TT> and cannot be changed from
     * inside of the program.
     *
     * \tparam ET Element type of matrices and vectors used by this solver.
     *            Must be either `double` or `std::complex<double>`.
     */
    template <typename ET>
    struct State {
        ...
        /// Free all memory allocated by PARDISO.
        /**
         * Needs to be called when the number of rows changes.
         * However, it is always called automatically when needed.
         */
        void clear() {
            if (_nrows > 0) {
                int error;
        ...
        double &operator[](const DParm dp) noexcept {
            return _dparm[static_cast<std::size_t>(dp)];
        }
        /// Access a double parameter.
        ...
        double *dparm() noexcept {
            return _dparm.get();
        }
        /// Return pointer to array of double parameters.
        const double *dparm() const noexcept {
            return _dparm.get();
        }
#endif
        ...
        std::vector<elementType> operator()(const int nrhs,
            ...
            (*this)(b.size(), nrhs, &a[0], &ia[0], &ja[0], &b[0],
                    &x[0], startPhase, endPhase);
            return x;
        }
        ...
        std::vector<elementType> operator()(const int nrhs,
            ...
            std::vector<elementType> x(b.size());
            (*this)(mat.rows(), nrhs, mat.a(), mat.ia(), mat.ja(),
        ...
        /**
         * Solves a*x = b for x.<BR>
         * This overload operates on blaze matrix and vector types.
         * It is the most convenient and safe to use but comes with some
         * overhead as the matrix needs to be copied into a Pardiso::Matrix
         * first.
         *
         * This overload handles only a single right hand side.
         *
         * \param mat Sparse matrix `a`.
         * \param b Right hand side vector.
         * \param startPhase Phase at which to start computation.
        ...
        blaze::DynamicVector<elementType> operator()(
            const blaze::CompressedMatrix<elementType> &mat,
            VT &b,
            const Phase startPhase=Phase::ANALYSIS,
            const Phase endPhase=Phase::SOLVE) {
            ...
            // solve equation
            blaze::DynamicVector<elementType> x(b.size());
            (*this)(mat.rows(), 1, pmat.a(), pmat.ia(), pmat.ja(),
                    &b[0], &x[0], startPhase, endPhase);
            return x;
        }
         ...
         * first.
         *
         * This overload handles multiple right hand sides stored in a matrix.
         *
         * \param mat Sparse matrix `a`.
        ...
        template <typename MT,
                  typename = std::enable_if_t<blaze::IsDenseMatrix<MT>::value, MT>>
        blaze::DynamicMatrix<elementType> operator()(
            const blaze::CompressedMatrix<elementType> &mat,
            MT &b,
            const Phase startPhase=Phase::ANALYSIS,
            const Phase endPhase=Phase::SOLVE) {
            ...
            blaze::DynamicMatrix<elementType> btrans = blaze::trans(b);
            // solve equation
            blaze::DynamicMatrix<elementType> x{btrans.rows(), btrans.columns()};
            (*this)(mat.rows(), btrans.rows(), pmat.a(), pmat.ia(), pmat.ja(),
